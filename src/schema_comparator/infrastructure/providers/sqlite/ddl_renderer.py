...
def format_sqlite_data_type(attrs: ColumnAttributes) -> str:
    ...
    if (
        ...
        and type_lower not in _SQLITE_NON_PARAMETRIC_TYPES
    ):
...
def format_sqlite_column_definition(col_name: str, attrs: ColumnAttributes) -> str:
    ...
    default_clause = (
        f" DEFAULT {attrs.default_expression}"
        if attrs.default_expression is not None
        else ""
    )
...
def generate_sqlite_script(
    ...
    discrepant_columns: Sequence[
        tuple[str, str, str, ColumnAttributes, ColumnAttributes]
    ],
) -> str:
    ...
    if missing_tables:
        ...
        for schema, table, columns in missing_tables:
            ...
            col_defs = [
                f"    {format_sqlite_column_definition(cname, attrs)}"
                for cname, attrs in columns
            ]
            lines.append(",\n".join(col_defs))
            lines.append(");")
            lines.append("")
    ...
    if discrepant_columns:
        ...
        lines.append(
            "-- 3. RECONSTRUCCIÓN DE TABLAS PARA COLUMNAS DISCREPANTES (TABLE REBUILD)"
        )
        lines.append(
            "-- ---------------------------------------------------------------------------"
        )
        rebuild_tables: dict[
            tuple[str, str], list[tuple[str, ColumnAttributes, ColumnAttributes]]
        ] = {}
        for schema, table, col_name, baseline_attrs, target_attrs in discrepant_columns:
            rebuild_tables.setdefault((schema, table), []).append(
                (col_name, baseline_attrs, target_attrs)
            )
        ...
        for (schema, table), col_discrepancies in rebuild_tables.items():
            ...
            lines.append(
                f"-- Rebuilding table {quoted_table} due to column attribute changes"
            )
