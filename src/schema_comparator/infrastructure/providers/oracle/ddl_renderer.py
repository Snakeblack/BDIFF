...
def format_oracle_data_type(attrs: ColumnAttributes) -> str:
    ...
    if attrs.character_maximum_length is not None and type_upper in (
        "VARCHAR2",
        "CHAR",
        "NVARCHAR2",
        "RAW",
...
def format_oracle_column_definition(col_name: str, attrs: ColumnAttributes) -> str:
    ...
    if is_ident:
        ...
        default_expr
        and "ISEQ$$" not in default_expr.upper()
        and ".NEXTVAL" not in default_expr.upper()
    ):
...
def generate_oracle_script(
    ...
    if missing_tables:
        ...
        for schema, table, columns in missing_tables:
            ...
            col_defs = [
                f"    {format_oracle_column_definition(cname, attrs)}"
