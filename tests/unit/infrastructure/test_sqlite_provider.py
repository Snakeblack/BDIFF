...
def test_sqlite_provider_validate_profile() -> None:
    ...
    valid_profile = ConnectionProfile(
        name="sqlite_dev", connection_string=":memory:", provider="sqlite"
    )
    provider.validate_profile(valid_profile)

    empty_profile = ConnectionProfile(
        name="sqlite_empty", connection_string="", provider="sqlite"
    )
    with pytest.raises(ValueError, match="empty connection string"):
        provider.validate_profile(empty_profile)

    wrong_provider = ConnectionProfile(
        name="sqlite_wrong", connection_string=":memory:", provider="postgres"
    )
...
def test_sqlite_provider_introspect_in_memory() -> None:
    ...
    profile = ConnectionProfile(
        name="sqlite_test", connection_string=":memory:", provider="sqlite"
    )

    conn = sqlite3.connect(":memory:")
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE users (id INTEGER PRIMARY KEY, username TEXT NOT NULL, email VARCHAR(255) NULL)"
    )
    cursor.close()

    with pytest.MonkeyPatch.context() as mp:
        mp.setattr(
            "schema_comparator.infrastructure.providers.sqlite.connection.connect",
            lambda prof, **kw: conn,
        )
...
def test_sqlite_ddl_renderer_format_column_definition() -> None:
    ...
    assert (
        ddl_renderer.format_sqlite_column_definition("username", col1)
        == '"username" VARCHAR(100) NOT NULL'
    )

    col2 = ColumnAttributes(
        data_type="NUMERIC",
    ...
    assert (
        ddl_renderer.format_sqlite_column_definition("price", col2)
        == '"price" NUMERIC(10, 2) NULL'
    )

    col3 = ColumnAttributes(
        data_type="INTEGER",
        character_maximum_length=None,
    ...
    assert (
        ddl_renderer.format_sqlite_column_definition("id", col3)
        == '"id" INTEGER NOT NULL'
    )
...
def test_sqlite_ddl_renderer_generate_script() -> None:
    profile = ConnectionProfile(
        name="sqlite_target", connection_string=":memory:", provider="sqlite"
    )
    ...
    missing_tables = [
        (
            ...
            "roles",
            [
                ("id", ColumnAttributes("INTEGER", None, 32, 0, False)),
                ("name", ColumnAttributes("TEXT", 50, None, None, False)),
            ],
        )
    ]
    missing_cols = [
    ...
    script = ddl_renderer.generate_sqlite_script(
