...
def test_postgresql_provider_validate_profile() -> None:
    ...
    empty_profile = ConnectionProfile(
        name="pg_empty", connection_string="", provider="postgresql"
    )
    ...
    wrong_provider = ConnectionProfile(
        name="pg_wrong", connection_string="host=localhost", provider="sqlserver"
    )
...
def test_postgresql_provider_introspect_success() -> None:
    ...
    with patch(
        "schema_comparator.infrastructure.providers.postgresql.connection.connect"
    ) as mock_connect:
...
def test_postgresql_translate_connect_import_error() -> None:
    err = errors.translate_connect_error(
        "pg_test", ImportError("No module named 'psycopg'")
    )
...
def test_postgresql_ddl_renderer_format_column_definition() -> None:
    ...
    assert (
        ddl_renderer.format_pg_column_definition("username", col1)
    ...
    assert (
        ddl_renderer.format_pg_column_definition("price", col2)
    ...
    assert (
        ddl_renderer.format_pg_column_definition("id", col3)
...
def test_postgresql_ddl_renderer_generate_script_selective_alter() -> None:
    ...
    missing_tables = [
        (
            ...
            [
                ...
                ("name", ColumnAttributes("varchar", 50, None, None, False)),
            ],
        )
    ]
    missing_cols = [
    ...
    script = ddl_renderer.generate_pg_script(
    ...
    assert (
        'ALTER TABLE "public"."users" ALTER COLUMN "bio" TYPE text USING CAST("bio" AS text);'
        in script
    )
