...
| 2 | Auth method | **Direct connection string per profile**, not separate server/user/password fields. Each microservice DB entry in the local config supplies its full ODBC connection string as-is (e.g. `Driver={ODBC Driver 18 for SQL Server};Server=...;Database=...;UID=...;PWD=...;` or `...;Trusted_Connection=yes;`), passed straight to `pyodbc.connect()`. The tool does not change auth *mode* — whatever the connection string expresses (SQL auth, Windows auth, `Encrypt=`, `TrustServerCertificate=`, etc.) is honored as-is, only keyword *spelling* is normalized (see below). **Superseded/revised (connection-string-translation, 2026-07-11):** the original "fully opaque, pass-through" description is narrowed. ADO.NET/`SqlClient`-style connection-string fragments (`Data Source=`, `Initial Catalog=`, `User Id=`, `Password=`, `Integrated Security=`) are now also accepted as valid profile input and translated once, at config-load time, into ODBC keyword form before a `ConnectionProfile` is constructed — so pasting a connection string straight from a microservice's `appsettings.json` works without manual rewriting. The string is still never decomposed into separate host/user/password/auth fields. | User explicitly requested direct connection-string access instead of a field-by-field auth model. This is also simpler and matches how developers already copy connection strings from their microservices' own `appsettings.json`/`.env` for local debugging — no need to decompose them into separate fields just to reassemble later. Still git-ignored, still never hardcoded (see Secrets below). |
